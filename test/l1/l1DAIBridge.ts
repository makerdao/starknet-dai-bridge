...
describe('L1DAIBridge', function () {
  ...
  describe('deposit', function () {
    it('escrows funds and sends a message to l2 on deposit', async () => {
      const {admin, l1Alice, dai, starkNetFake, escrow, l1Bridge, l2BridgeAddress} = await setupTest();
      ...
      await l1Bridge.connect(l1Alice).deposit(l1Alice.address, l2User, depositAmount)
      ...
      expect(starkNetFake.sendMessageToL2).to.have.been.calledWith(
        l2BridgeAddress, DEPOSIT_SELECTOR, [l2User, depositAmount]
      );
    })
    it('reverts when approval is too low', async () => {
      ...
      await dai.connect(admin).transfer(l1Alice.address, depositAmount)
      await dai.connect(l1Alice).approve(l1Bridge.address, depositAmount.sub(1))
  ...
  describe('finalizeWithdrawal', function () {
    it('sends funds from the escrow', async () => {
      ...
      await l1Bridge.connect(l1Alice).finalizeWithdrawal(l1Alice.address, withdrawalAmount)
      ...
      expect(starkNetFake.consumeMessageFromL2).to.have.been.calledWith(
        l2BridgeAddress, [MESSAGE_WITHDRAW, l1Alice.address, withdrawalAmount]
      );
    })
    it('sends funds from the escrow to the 3rd party', async () => {
      const {admin, l1Alice, l1Bob, dai, starkNetFake, escrow, l1Bridge, l2BridgeAddress} = await setupTest();
      ...
      await l1Bridge.connect(l1Alice).finalizeWithdrawal(l1Bob.address, withdrawalAmount)
    ...
    it('reverts when called by not a withdrawal recipient', async () => {
      ...
      starkNetFake.consumeMessageFromL2
        .whenCalledWith(l2BridgeAddress, [MESSAGE_WITHDRAW, l1Bob.address, withdrawalAmount])
        .reverts()

      await expect(
        l1Bridge.connect(l1Bob).finalizeWithdrawal(l1Alice.address, withdrawalAmount)
      ).to.be.reverted

      expect(starkNetFake.consumeMessageFromL2).to.have.been.calledWith(
        l2BridgeAddress, [MESSAGE_WITHDRAW, l1Bob.address, withdrawalAmount]
      );
    })
    it('reverts when called with wrong amount', async () => {
      ...
      starkNetFake.consumeMessageFromL2
        .whenCalledWith(l2BridgeAddress, [MESSAGE_WITHDRAW, l1Alice.address, wrongAmount])
        .reverts()

      await expect(
        l1Bridge.connect(l1Alice).finalizeWithdrawal(l1Alice.address, wrongAmount)
      ).to.be.reverted

      expect(starkNetFake.consumeMessageFromL2).to.have.been.calledWith(
        l2BridgeAddress, [MESSAGE_WITHDRAW, l1Alice.address, wrongAmount]
      );
    })
    it('reverts when escrow access was revoked', async () => {
      ...
      await expect(
        l1Bridge.connect(l1Alice).finalizeWithdrawal(l1Alice.address, withdrawalAmount)
      ).to.be.revertedWith('ERC20: transfer amount exceeds allowance')
    })
  })
  ...
  testAuth({
    ...
    getDeployArgs: async () => {
      ...
      return [starkNetFake.address, dai.address, escrow.address, l2BridgeAddress]
    },
...
async function setupTest() {
  ...
  const l1Bridge = await simpleDeploy(
    'L1DAIBridge',
    [starkNetFake.address, dai.address, escrow.address, L2_DAI_BRIDGE_ADDRESS]
  );
