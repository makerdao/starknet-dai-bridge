...
import {
  ...
  getActiveWards,
  getAddress,
  getAddressOfNextDeployedContract, getL1ContractAt,
  getL2ContractAt,
  getNetwork,
  getOptionalEnv,
  getRequiredEnv,
...
task("deploy-bridge-upgrade", "Deploy bridge upgrade").setAction(async (_, hre) => {
  ...
  const l2DAI = await getL2ContractAt(hre, "dai", getRequiredEnv(`${NETWORK}_L2_DAI_ADDRESS`))

  const l2GovernanceRelay = await getL2ContractAt(
    hre, "l2_governance_relay", getRequiredEnv(`${NETWORK}_L2_GOVERNANCE_RELAY`)
  )

  const registry = await getL2ContractAt(hre, "registry", getRequiredEnv(`${NETWORK}_REGISTRY_ADDRESS`))
  const l1Escrow = await getL1ContractAt(hre, "L1Escrow", getRequiredEnv(`${NETWORK}_L1_ESCROW`))
  ...
  if (DENY_DEPLOYER) {
    await waitForTx(l1DAIBridge.deny(await l1Signer.getAddress(), gasOverrides));
  }
