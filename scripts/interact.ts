...
import { getAddress, getRequiredEnvDeployer, parseCalldataL1, parseCalldataL2 } from "./utils";
...
task("invoke:l2", "Invoke an L2 contract")
  ...
  .setAction(async ({ contract, func, calldata, name }, hre) => {
    ...
    const ECDSA_PRIVATE_KEY = getRequiredEnvDeployer(`${_name.toUpperCase()}_ECDSA_PRIVATE_KEY`);
...
task("call-oracle", "Call an L1 contract")
  .setAction(async (_, hre) => {
    const [l1Signer] = await hre.ethers.getSigners();

    const oracleAuth = new Contract(
      '0x70FEdb21fF40E8bAf9f1a631fA9c34F179f29442',
      new Interface(['function requestMint((bytes32,bytes32,bytes32,bytes32,uint128,uint80,uint48),bytes,uint256,uint256)', 'function getGUIDHash((bytes32,bytes32,bytes32,bytes32,uint128,uint80,uint48)) view returns (bytes32)', 'function signers(address) view returns(uint256)']),
      l1Signer,
    );
