...
describe('escrow', () => {
    describe('approve', () => {
        it('sets approval on erc20 tokens', async () => {
            ...
            expect(await dai.allowance(escrow.address, l1Alice.address)).to.be.eq(allowanceLimit)
        })
        ...
        it('reverts when called by unauthed user', async () => {
            ...
            await expect(
              escrow.connect(l1Alice).approve(dai.address, l1Bob.address, allowanceLimit),
            ).to.be.revertedWith('L1Escrow/not-authorized')
        })
    })

    it('has correct public interface', async () => {
        await assertPublicMutableMethods(
          'L1Escrow',
          [
            'rely(address)',
            'deny(address)',
