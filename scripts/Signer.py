...
from starkware.crypto.signature.signature import private_to_stark_key, sign, verify, EC_GEN, FIELD_PRIME
...
class Signer():
    ...
    def verify(self, message_hash, r, s):
        return verify(message_hash, r, s, self.public_key)
...
signer = Signer(int(sys.argv[1]))
msg_hash = hash_message(
    int(sys.argv[2], 16), # acccount address
    int(sys.argv[3], 16), # contract address
    int(sys.argv[4]), # selector
